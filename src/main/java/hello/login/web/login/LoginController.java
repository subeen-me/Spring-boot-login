...
public class LoginController {
   ...
   // @PostMapping("/login")
    public String login(@Valid @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletResponse response) {
        ...
        if(loginMember == null) { //글로벌 오류 처리. bindingResult.reject 이용
  ...
  //  @PostMapping("/logout")
    ...
    public String logoutV3(HttpServletRequest request) {
        HttpSession session = request.getSession(false); //true로 하면 세션이 없으면 다시 만들어버리기 때문에 logout에는 false
        if(session != null) {
            session.invalidate(); //세션과 데이터가 다 없어진다.
        }
